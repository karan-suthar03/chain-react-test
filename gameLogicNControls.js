...
function check(){
    ...
    if(matrix != undefined && matrix != null && matrix.length != 0){
        console.log(8437387328374,matrix)
        game.child('matrix').set(matrix);
    }
    //game.child('matrix').set(matrix);
    for(i= 0 ;i<cols;i++){
        for(j = 0;j<rows;j++){
...
function mousePressed() {
    ...
    if (!isPressed && !isok && !isRun) {
        ...
        console.log(i,j)
        if ((i < cols && i > 0 || Object.is(i, 0)) && (j < rows && j > 0 || Object.is(j, 0)) && player == myID) {
            if(matrix[i][j].player == player || matrix[i][j].balls == 0){
                ...
                isok = true;
                some = true;
                game.child('click').set({player:myID,click:[i,j]})
            }
        }
        check();
    }
